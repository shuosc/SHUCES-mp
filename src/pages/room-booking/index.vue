<template lang="pug">
  div.page()
    ...
    div(style="box-shadow: 0 2px 5px rgba(0, 0, 0, 0.2);border-radius:10px;margin:10px 10px 5px 10px;")
      div.ces.booking-header(style="") 选择预约日期
      div.ces.date
        div(:class="{'border-date':dayIndex===index}" style="flex:1" v-for="(date,index) in dates" :key="date.md",@click="onDateIndexClick(index)")
          p
            ...
            br
            | {{date.md}}
    div(style="box-shadow: 0 2px 5px rgba(0, 0, 0, 0.2);border-radius:10px;margin:5px 10px 10px 10px;")
      div.ces.booking-header(style="") 房间使用表 {{date.md}} 周{{date.dd}}
      //- div.ces.date
      //-   div(:class="{'border-date':dayIndex===index}" style="flex:1" v-for="(date,index) in dates" :key="date.md",@click="onDateIndexClick(index)")
      //-     p
      //-       | 周{{date.dd}}
...
export default {
  ...
  created: function() {
    ...
    let datePointer = now.subtract(3, 'd')
    for (let i = 0; i < 7; i++) {
      this.dates.push({
  ...
  methods: {
    ...
    finishOrder(id) {
      this.$http
        .put(`/room-booking/orders/${id}`, { end: '' })
        .then(resp => {
          wx.showToast({
            title: '本次预约已结束，感谢您的使用',
            icon: 'success',
            duration: 2000
          })
          this.getHistory()
