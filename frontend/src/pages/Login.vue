    ...
    .content
      q-card.no-margin(square='', flat='', style='position:absolute;top:-5rem;width:100%;')
        q-card-main(style='text-align:center;')
          span(style='font-weight:bold;font-size:2rem;color:white;text-shadow:yellow 0 1px 0;') 上海大学计算机学院
          br
      ...
      this.$http
        ...
        .then(response => {
          ...
          try {
            ...
            this.$q.notify('本地存储失败，如果在使用浏览器，请关闭无痕浏览模式')
          }
          _this.$store.commit('user/updateUser', payload)
          _this.$q.notify(`${response.data.name}，欢迎登录`)
          ...
          if (_this.$route.query.redirect) {
            ...
            _this.$router.push('/room-booking/new')
          }
        })
        .catch(function(error) {
          console.log(error)
          _this.$q.notify('登录失败' + error)
        })
    }
  }
}
